...
import type { FluentBatchApi, FluentWidgetSession } from "@fluent.xyz/connect";
import { encodeFunctionData, formatUnits, parseUnits, type Address, type Hash } from "viem";
import { blendAbi } from "../contracts";
import { BLEND_TOKEN, EXPLORER_BASE_URL, ONE_BLEND } from "../consts";
...
export function TransferPanel({
  ...
  const txUrl = useMemo(() => (txHash ? `${EXPLORER_BASE_URL}/tx/${txHash}` : null), [txHash]);
  ...
  useEffect(() => {
    ...
    const shouldMirror = (args: unknown[]) =>
      args.some(
        (arg) =>
          typeof arg === "string" && /\[(fluent|hosted|privy|zerodev)/i.test(arg),
      );
  ...
  async function sendOneBlendToSelf() {
    ...
    appendLog(`Preparing 1 BLEND self-transfer with ${gasPayment.symbol} gas payment.`);
    try {
        ...
        calls: [
          {
            ...
            data: encodeFunctionData({
              ...
              args: [smartAccountAddress, ONE_BLEND],
            }),
          },
        ],
      });
  ...
  return (
    <section className="transfer-panel">
      ...
      <p>
        Sends {formatUnits(ONE_BLEND, BLEND_TOKEN.decimals)} BLEND from the ZeroDev smart
        account back to itself. Gas is charged through the selected ERC20 paymaster route.
      </p>
