...
The chess demo exposes this through the `Test gas route` button.
...
- gas token: USDnr or BLEND, selected in that priority order

It sends a zero-value no-op call to `0x000000000000000000000000000000000000dEaD`.

The demo includes a ZeroDev-generated approval call for the selected ERC20 paymaster token. The caller must provide an explicit approval amount, and the spender address is resolved by ZeroDev rather than hardcoded.
