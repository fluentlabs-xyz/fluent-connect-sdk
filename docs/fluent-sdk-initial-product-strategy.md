...
The app should not need to pass a `clientId` in the default builder path. Fluent Connect derives app identity from origin and an SDK-managed installation id. Registered `clientId` remains an advanced production override for stricter app controls.
...
App registration remains in Postgres for production apps. The registry is operational state, not analytics state, and should continue to own registered `clientId`, origins, scopes, faucet configuration, campaign metadata, and display configuration. Hackathon/basic integrations should be able to start without registration.
...
- External wallet options are routed through the SDK-owned Reown AppKit bridge instead of builder-owned wallet plumbing.
- Fluent ID stays a first-class action in the SDK connect choice sheet.
